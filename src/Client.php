...
class Client
{
    ...
    /**
     * Makes request to Dividend Core.
     *
     * @param string $method
     * @param mixed  $params
     *
    ...
    public function request($method, ...$params)
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\DividenddException($response->error());
            }
    ...
    /**
     * Makes async request to Dividend Core.
     *
     * @param string        $method
     * @param mixed         $params
    ...
    protected function defaultConfig(array $config = [])
    {
        $defaults = [
            ...
            'host'       => '127.0.0.1',
            'port'       => 19998,
            'user'       => 'dividendcashrpc',
            'password'   => '5pdjzTZrjsuJk6ivFSD65phgWZXQ5aCtXB4PWrvaFAUa',
        ];

        // use same var name as laravel-dividendrpc
        if (
    ...
    protected function getHandler()
    {
        ...
        $stack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return DividenddResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    protected function onSuccess(ResponseInterface $response, callable $callback = null)
    {
        if (!is_null($callback)) {
            if ($response->hasError()) {
                $response = new Exceptions\DividenddException($response->error());
            }
    ...
    protected function handleException($exception)
    {
        if ($exception->hasResponse()) {
            ...
            if ($response->hasError()) {
                return new Exceptions\DividenddException($response->error());
            }
        }
